...
public class EvaluationController {
	...
	public ModelAndView saveEditQuestion(@ModelAttribute("question") Question q){
	  ...
	  private ModelAndView validateQuestion(Question q, ModelAndView model, boolean add){
	    	model.setViewName("editQuestion");
	    	IntegerValidator integerValidator = new IntegerValidator();
	    	
	    	if (q.getLibelle() == null || q.getPonderation() == null || q.getValMax() == null){
	    		model.addObject("error", "Vous devez remplir tous les champs !");
	    	}
	    	else if(!integerValidator.isInRange(q.getValMax(), 0, 10)){
	    		model.addObject("error", "La valeur de la note doit être comprise entre 0 et 10");
	    	}
	    	else if(!integerValidator.isInRange(q.getPonderation(), 0, 10)){
	    		model.addObject("error", "La valeur de l'importance de la question doit être comprise entre 0 et 10");
	    	}
	    	else {
	    		if (add){
	    			...
	    			model.addObject("msg", "La question a correctement été modifiée");
	    		}
